...
describe('Persisted schema version guards', () => {
  beforeEach(() => {
    localStorageMock.clear()
  })

  it('returns null when tabs schema version is newer than this client understands', () => {
    localStorage.setItem('freshell.tabs.v1', JSON.stringify({
      version: 999,
      tabs: {
        tabs: [],
  ...
  it('returns null when panes schema version is newer than this client understands', () => {
    localStorage.setItem('freshell.panes.v1', JSON.stringify({
      version: 999,
      layouts: {},
      activePane: {},
    ...
    expect(loadPersistedPanes()).toBeNull()
  })
})
...
describe('Persisted state repair', () => {
  ...
  it('recovers tabs from pane layouts when tabs are missing', () => {
    localStorage.setItem('freshell.panes.v1', JSON.stringify({
      version: 4,
      layouts: {
        'tab-1': {
    ...
    const recovered = loadPersistedTabs()
    expect(recovered).not.toBeNull()
    expect(recovered.version).toBe(1)
    expect(recovered.tabs.activeTabId).toBe('tab-1')
    expect(recovered.tabs.tabs).toHaveLength(1)
    expect(recovered.tabs.tabs[0].id).toBe('tab-1')
    expect(recovered.tabs.tabs[0].title).toBe('example.com')

    // Should persist the recovered tabs back to storage for next load.
    const raw = localStorage.getItem('freshell.tabs.v1')
    expect(raw).not.toBeNull()
  })

  it('prunes orphaned pane layouts that do not have corresponding tabs', () => {
    localStorage.setItem('freshell.tabs.v1', JSON.stringify({
      version: 1,
      tabs: {
        tabs: [{ id: 'tab-1', title: 'Tab 1', createdAt: 1 }],
        activeTabId: 'tab-1',
      },
    }))
    ...
    localStorage.setItem('freshell.panes.v1', JSON.stringify({
      ...
      layouts: {
        ...
        'tab-orphan': { type: 'leaf', id: 'pane-orphan', content: { kind: 'terminal', mode: 'shell' } },
      },
      activePane: { 'tab-1': 'pane-1', 'tab-orphan': 'pane-orphan' },
      paneTitles: { 'tab-orphan': { 'pane-orphan': 'Orphan' } },
      paneTitleSetByUser: { 'tab-orphan': { 'pane-orphan': true } },
    }))

    const loaded = loadPersistedPanes()
    expect(Object.keys(loaded.layouts)).toEqual(['tab-1'])
    expect(loaded.activePane['tab-orphan']).toBeUndefined()
    expect(loaded.paneTitles['tab-orphan']).toBeUndefined()
    expect(loaded.paneTitleSetByUser['tab-orphan']).toBeUndefined()
  })

  it('repairs activePane when it points to a non-existent pane id', () => {
    localStorage.setItem('freshell.panes.v1', JSON.stringify({
      version: 4,
      layouts: {
        'tab-1': {
          ...
          children: [
            ...
            { type: 'leaf', id: 'pane-2', content: { kind: 'terminal', mode: 'shell' } },
          ],
        },
      },
      activePane: { 'tab-1': 'missing-pane' },
      paneTitles: {},
      paneTitleSetByUser: {},
    }))

    const loaded = loadPersistedPanes()
    expect(loaded.activePane['tab-1']).toBe('pane-2')
  })
})
